...
namespace ipc {
    ...
    PythonBackendController::PythonBackendController(std::string pythonExecutable, std::string pythonScript)
        : m_pythonExecutable(std::move(pythonExecutable)),
          m_pythonScript(std::move(pythonScript)),
          m_shutdownContext(1) {
    }
    ...
    void PythonBackendController::launchProcess() const {
        ...
        HINSTANCE hRes = ShellExecuteA(nullptr, "open", m_pythonExecutable.c_str(),
                                         m_pythonScript.c_str(), nullptr, SW_HIDE);
        auto code = reinterpret_cast<intptr_t>(hRes);
        if (code <= 32) {
    ...
    void PythonBackendController::launchBackend() const {
        launchProcess();
    }
