...
bool ScreenshotSaver::saveBitmapToFile(const std::string                        &filename,
                                       const ScreenshotProcessor::BITMAPINFO256 &bmpInfo256,
    ...
    spdlog::info("Expected file size: {} bytes (Header: {} + Pixel Data: {})", fileHeader.bfSize,
                 sizeof(BITMAPFILEHEADER) + sizeof(BITMAPINFOHEADER), pixelData.size());
...
std::vector<BYTE> ScreenshotSaver::createBitmapBuffer(const ScreenshotProcessor::BITMAPINFO256 &bmpInfo256,
                                                      const std::vector<BYTE>                  &pixelData) {
