...
std::vector<BYTE> ScreenshotProcessor::convertToGrayscale(HBITMAP hBitmap, int width, int height,
    ...
    const int result = GetDIBits(hDC, hBitmap, 0, height, pixelData.data(), reinterpret_cast<BITMAPINFO *>(&bmpInfo256),
                                 DIB_RGB_COLORS);
    ...
    spdlog::info("Grayscale conversion executed successfully. Pixels extracted: {} (expected: {})", pixelData.size(),
                 width * height * (GRAYSCALE_BIT_DEPTH / 8));
